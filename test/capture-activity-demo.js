...
(async () => {
  const browser = await puppeteer.launch({
    ...
    args: ['--no-sandbox', '--disable-setuid-sandbox', '--user-data-dir=/tmp/puppeteer-activity-' + Date.now()]
  });
  ...
  // Open activity panel
  await page.click('#activity-toggle-btn');
  await new Promise(r => setTimeout(r, 1500));

  await page.screenshot({ path: path.join(OUT, 'activity-panel.png') });
  console.log('Captured: activity panel');
