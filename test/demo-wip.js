...
(async () => {
  ...
  // Screenshot 1: Board with WIP badges
  await page.screenshot({ path: 'docs/gifs/wip-board.png', fullPage: false });
  console.log('Saved: docs/gifs/wip-board.png');

  // Open settings
  await page.evaluate(() => document.getElementById('settings-btn').click());
  await new Promise(r => setTimeout(r, 1500));

  // Scroll to WIP section
  await page.evaluate(() => {
    const modal = document.querySelector('.settings-content');
    const wipSection = document.getElementById('wip-limits-section');
    if (modal && wipSection) {
      wipSection.scrollIntoView({ behavior: 'instant', block: 'center' });
    }
  });
  await new Promise(r => setTimeout(r, 500));

  // Screenshot 2: Settings with WIP limits
  await page.screenshot({ path: 'docs/gifs/wip-settings.png', fullPage: false });
  console.log('Saved: docs/gifs/wip-settings.png');
