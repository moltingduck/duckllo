...
func (s *Store) AppendIteration(ctx context.Context, runID uuid.UUID, phase, role, provider, model, summary, transcriptURL, transcript string) (*models.Iteration, error) {
	...
	if err := tx.QueryRow(ctx, `
		INSERT INTO iterations (run_id, idx, phase, agent_role, provider, model, summary, transcript_url, transcript)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id, run_id, idx, phase, agent_role, provider, model, summary, transcript, transcript_url,
		          prompt_tokens, completion_tokens, status, started_at, finished_at
	`, runID, nextIdx, phase, role, provider, model, summary, transcriptURL, transcript).Scan(
		&it.ID, &it.RunID, &it.Idx, &it.Phase, &it.AgentRole, &it.Provider, &it.Model,
		&it.Summary, &it.Transcript, &it.TranscriptURL,
		&it.PromptTokens, &it.CompletionTokens, &it.Status,
...
func (s *Store) UpdateIteration(ctx context.Context, id uuid.UUID, p IterationPatch) (*models.Iteration, error) {
	...
	err := s.Pool.QueryRow(ctx, `
		...
		RETURNING id, run_id, idx, phase, agent_role, provider, model, summary, transcript, transcript_url,
		          prompt_tokens, completion_tokens, status, started_at, finished_at
	`, id, p.Summary, p.Transcript, p.PromptTokens, p.CompletionTokens, p.Status).Scan(
		&it.ID, &it.RunID, &it.Idx, &it.Phase, &it.AgentRole, &it.Provider, &it.Model,
		&it.Summary, &it.Transcript, &it.TranscriptURL,
		&it.PromptTokens, &it.CompletionTokens, &it.Status,
		&it.StartedAt, &it.FinishedAt,
...
func (s *Store) ListIterations(ctx context.Context, runID uuid.UUID) ([]models.Iteration, error) {
	rows, err := s.Pool.Query(ctx, `
		SELECT id, run_id, idx, phase, agent_role, provider, model, summary, transcript, transcript_url,
		       prompt_tokens, completion_tokens, status, started_at, finished_at
